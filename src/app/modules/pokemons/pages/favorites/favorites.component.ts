...
@Component({
  ...
  standalone: true,
  imports: [MatIconModule, NgFor, NgStyle, MatButtonModule, MatIconModule, ReactiveFormsModule, NgClass, MatInputModule],
  templateUrl: './favorites.component.html',
  styleUrl: './favorites.component.scss'
})
export class FavoritesComponent {
  private favorites: Pokemon[] = JSON.parse(localStorage.getItem('favorites')!) ?? [];
  ...
  protected filterPokemons(): void {
    ...
    if (this.pokemonFilter.value) {
      this.displayedFavorites = favorites.filter(pokemon => pokemon.name.toLowerCase().includes(this.pokemonFilter.value.toLowerCase()));
    } else {
