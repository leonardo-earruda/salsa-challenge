...
import { Pokemon, PokemonSpecies } from '../../../../shared/utils/pokemon.model';
...
@Component({
  ...
  standalone: true,
  imports: [MatProgressBar, MatIconModule, MatButtonModule, TitleCasePipe, MatCardModule, NgClass],
...
export class DetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(private activatedRoute: ActivatedRoute, private pokemonService: PokemonsService, private location: Location, private elementRef: ElementRef) {
  ...
  private getPokemon(): void {
    this.pokemonService.getPokemonById(this.pokemonId).subscribe((pokemon: Pokemon) => {
  ...
  private extractUrlsAndNames(data: any): void {
    if (data.species) {
      this.urlsAndNames.push({ url: data?.species?.url, name: data?.species?.name, minLevel: data?.evolution_details[0]?.min_level});
    }
  ...
  private displayEvolutionChain(evolutions: Evolutions[]): void {
    for(let evolution of evolutions) {
      this.pokemonService.getByUrl(evolution.url.replace('-species', ''))
      .pipe(
  ...
  protected changeColors(clickedColor?: string) {
    ...
    rootElement.style.setProperty('--primary-color', clickedColor ? this.getColor(clickedColor) : this.getColor());
  }

  private getColor(clickedColor?: string): string {
    let color = null;
    if(clickedColor) {
      color = this.pokemon.types.find(type => type.type.name.toLocaleLowerCase() === clickedColor.toLocaleLowerCase())?.type.name;
    } 
    return this.pokemonService.getPokemonColor(color ? color : this.pokemon.types[0].type.name);
  }
